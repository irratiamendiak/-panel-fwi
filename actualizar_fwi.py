...
def descargar(cli, sensores, datos, dia_fin, primera, hora, max_dias, alm=None):
    ...
    def previsor(estados, ref, ahora):
        ...
        for nombre, (lat, lon, alt) in COORD.items():
            ...
            while dia <= hoy + timedelta(days=3):
                ...
                medidas = 0
                if j == 0:         # hoy, antes de que llegue el dato de las 12:00: la lluvia ya caída se toma de la estación
                    try:
...
def main() -> int:
    ...
    alm = ew.Almacen(cli)
    nuevas = descargar(cli, sensores, datos, dia_fin, primera, a.hora, a.max_dias, alm)

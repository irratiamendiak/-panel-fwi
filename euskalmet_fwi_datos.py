...
def obtener_dia(cli, alm, sensores, cod, dia, hora, minimo=MIN_LLUVIA):
    ...
    if datos is None:
        ...
        # La dirección no aparece en el catálogo de resúmenes diarios (no tiene sentido resumir un
        # ángulo con media o máximo), así que no se puede detectar como el resto de sensores. Se prueba
        # con el sensor de viento, que suele medir también la dirección (misma veleta). Si la prueba no
        # devuelve un dato real, no se guarda y se reintenta al día siguiente.
        try:
            nuevos = detectar_sensores(cli, cod, dia)
        except RuntimeError:
            nuevos = None
        candidato = (nuevos or {}).get("direccion")
        if not candidato and "viento" in sensores[cod]:

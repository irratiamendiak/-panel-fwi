...
def obtener_dia(cli, alm, sensores, cod, dia, hora, minimo=MIN_LLUVIA):
    ...
    if datos is None:
        ...
        # la dirección puede haberse añadido más tarde que el resto de sensores: se revisa una vez al día
        try:
            nuevos = detectar_sensores(cli, cod, dia)
            if nuevos.get("direccion"):
                sensores[cod]["direccion"] = nuevos["direccion"]

...
def descargar_horario(lat, lon, alt, reintentos=4, espera=25):
    ...
    for i in range(reintentos):
        ...
        if r.status_code == 200:
            h = r.json().get("hourly", {})
            t = h.get("time", [])
            cols = [h.get(k) for k in ("temperature_2m", "relative_humidity_2m", "wind_speed_10m", "precipitation")]
...
def entradas_dia(tabla, dia, hora=12):
    """(T, HR, viento, lluvia24h, horas_lluvia) de un día; None si falta algún dato.
    ...
    for i in range(24):
        ...
        x = tabla.get(m)
        if not x or x[3] is None:
            return None
        horas.append((m, float(x[3])))
    return (v[0], v[1], v[2], sum(p for _, p in horas), horas)
